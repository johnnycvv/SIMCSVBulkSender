...
class SmsSenderService : Service() {
        ...
        serviceScope.launch {
            ...
            while (isRunning && SmsQueue.hasPending()) {
                ...
                if (settings.sessionBatchLimit > 0 && sessionBatchSentCount >= settings.sessionBatchLimit) {
                    isRunning = false
                    updateNotification("Session limit reached.")
    ...
    private fun sendSmsWithCallbacks(phone: String, message: String, name: String, jobId: Long, subscriptionId: Int) {
        try {
            ...
            val sentPI = PendingIntent.getBroadcast(
                this, jobId.toInt(),
                Intent(ACTION_SMS_SENT, null, this, SmsSentReceiver::class.java).also { it.putExtras(baseExtras) },
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )

            val deliveredPI = PendingIntent.getBroadcast(
                this, (jobId + 1_000_000).toInt(),
                Intent(ACTION_SMS_DELIVERED, null, this, SmsDeliveryReceiver::class.java).also { it.putExtras(baseExtras) },
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
            ...
            if (parts.size == 1) {
                ...
                smsManager.sendMultipartTextMessage(phone, null, parts,
                    ...
                    ArrayList(List(parts.size) { deliveredPI }))
            }
    ...
    private fun resolveSmsManager(subscriptionId: Int): SmsManager {
        return if (subscriptionId != SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                applicationContext.getSystemService(SmsManager::class.java).createForSubscriptionId(subscriptionId)
            } else {
    ...
    private suspend fun getTodaySentCount(): Int = try {
        AppDatabase.getInstance(applicationContext).smsLogDao().getTodaySentCount()
    } catch (_: Exception) { 0 }

    private fun computeDelay(): Long = if (settings.randomizeDelay) {
    ...
    private fun getSubscriptionId(): Int = try {
        val sm   = getSystemService(Context.TELEPHONY_SUBSCRIPTION_SERVICE) as SubscriptionManager
        val subs = sm.activeSubscriptionInfoList
        subs?.getOrNull(settings.selectedSimSlot)?.subscriptionId ?: SubscriptionManager.INVALID_SUBSCRIPTION_ID
    } catch (_: SecurityException) { SubscriptionManager.INVALID_SUBSCRIPTION_ID }
    ...
    private fun buildNotification(text: String): Notification {
        ...
        val stopPI = PendingIntent.getService(this, 1,
            Intent(this, SmsSenderService::class.java).apply { action = ACTION_STOP }, PendingIntent.FLAG_IMMUTABLE)
        val (togglePI, toggleLabel) = if (isPaused)
            PendingIntent.getService(this, 2,
                Intent(this, SmsSenderService::class.java).apply { action = ACTION_RESUME },
                PendingIntent.FLAG_IMMUTABLE) to "Resume"
        else
            PendingIntent.getService(this, 2,
                Intent(this, SmsSenderService::class.java).apply { action = ACTION_PAUSE },
                PendingIntent.FLAG_IMMUTABLE) to "Pause"
